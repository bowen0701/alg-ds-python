...
def make_change_recur(amount, coins, n):
    ...
    count_in = make_change_recur(amount - coins[n], coins, n)
    count_ex = make_change_recur(amount, coins, n - 1)
    count = count_in + count_ex
    return count


def _make_change_memo(amount, coins, T, n):
    ...
    count_in = _make_change_memo(amount - coins[n - 1], coins, T, n)
    count_ex = _make_change_memo(amount, coins, T, n - 1)
    T[n - 1][amount] = count_in + count_ex

    return T[n - 1][amount]


def make_change_memo(amount, coins, n):
    ...
    return _make_change_memo(amount, coins, T, n)
...
def main():
    ...
    print('Make change by recursion: {}'
          .format(make_change_recur(amount, coins, n)))
    print('Time: {}'.format(time.time() - start_time))

    start_time = time.time()
    print('Make change by memo: {}'
          .format(make_change_memo(amount, coins, n)))
    print('Time: {}'.format(time.time() - start_time))
