...
def _previsit(v, previsited_d, clock, ccid, ccid_d):
    clock[0] += 1
...
def _dfs_explore(v, graph_adj_d, visited_d, 
    ...
    _previsit(v, previsited_d, clock, ccid, ccid_d)
...
def strongly_connected_components(graph_adj_d):
    ...
    rev_ccid_d, rev_revisited_d, rev_postvisited_d = dfs(rev_graph_adj_d)
    dec_rev_postvisited_ls = _decrease_postvisit_vertices(rev_postvisited_d)
    ...
    # Run DFS in the decreasing order of reverse graph's postvisit number. 
